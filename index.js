...
const myPepperoni = document.getElementById("myPepperoni")
...
function calculateTotal(){
    myResult = 0;
    if(mySmall.checked){myResult += 6}
    if(mySausage.checked){myResult += 2 }
    if(myPepperoni.checked){myResult += 3 }
    if(myPinapple.checked){myResult += 4 }
    if(myMedium.checked){myResult += 9}
    if(myLarge.checked) {myResult += 12}

    myTotal.innerHTML = "Your total is $" + myResult + ".00"
    myThanks.innerHTML = "Thank you for your order!"
